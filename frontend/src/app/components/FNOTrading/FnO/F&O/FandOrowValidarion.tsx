...
const Fandorow = forwardRef(({
    ...
    const handleDropdownChange = (index, value) => {
        ...
        updatedLegs[index].target = value;
        setlegs(updatedLegs);
    };
    ...
    const handleDelete = (indexToDelete) => {
        setlegs((prevLegs) => {
            ...
            const updatedLegs = prevLegs.filter((_, index) => index !== indexToDelete);
            return updatedLegs;
        });
    };
    ...
    useEffect(() => {
        if (
            ...
            legs.map((leg) => leg["option_type"]).join(",") !== allOptionTypes.join(",") ||
            legs.map((leg) => leg.expiry_date).join(",") !== allExpiryDates.join(",")
        ) {
    ...
    const get_leg_ltp = async () => {
        for (let index = 0; index < allStrikeValues.length; index++) {
            ...
            if (
                ...
                try {
                    const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_HOST}/get_price_details/${mainUser}`, {
                        method: "POST",
                        headers: {
            ...
            if (index === allStrikeValues.length - 1) {
                setgotAllLTP(true);
                let lotsize = stock_symbol === "NIFTY"
                    ? 25
                ...
                legs.map((leg) => {
                    ...
                    margin = margin + lotsize.toFixed(2) * Number(leg["lots"]).toFixed(2) * Number(ltp).toFixed(2);
                });
