...
const SettingsPanel: React.FC<{ onClose: () => void; onAddUser: (formData: Record<string, unknown>) => void }> = ({  onClose,
  ...
  const handleToggleInputVisibility = (e, key) => {
    ...
    {Object.keys(formData)
      ...
      .map((key, idx) => (
        ...
        >
          ...
          ></div>
          {key !== 'DataAPI' && key !== 'AutoLogin' && key !== 'QtyOnMaxLossPerTrade' && key !== 'TradingAuthorizationReq' && key !== 'EnableCNCsqOff' && key !== 'TradingAuthoReq' && !visibleInputs[key] ? (
            <button
...
{showProfitSettings && (
  <div style={{
    ...
    <ProfitSettingsForm onSubmit={handleProfitSettingsSubmit} />
  </div>
)}
        {showProfitSettings && (
        <div
